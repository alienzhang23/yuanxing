...
$(document).ready(function () {
    ...
    $('.select-box li').on('click', function (e) {
        if (!e.currentTarget.attributes.name) {
            console.log($(e.currentTarget).siblings('li[name="all"]')[0].className)
            $(e.currentTarget).siblings('li[name="all"]')[0].className = '';
        }
        if (e.currentTarget.className == '') {
            e.currentTarget.className = 'active-btn';
            if (e.currentTarget.attributes.name) {

                for (let i of $(e.currentTarget).siblings()) {
                    i.className = '';
                }
            }
        } else {
